...
class PostgreSQL implements QueryBuilderInterface
{
    ...
    {
        $this->query .= " OR \"$column\" ILIKE :$column";
        $this->parameters[":$column"] = "%$value%";
        return $this;
    }
    ...
    {
        if(!str_contains($this->query, 'WHERE')) {
            $this->query .= " WHERE \"$column\" NOT ILIKE '%$value%'";
        } else {
            $this->query .= " AND \"$column\" NOT ILIKE '%$value%'";
        }
    ...
    {
        $this->query .= " OR \"$column\" NOT ILIKE '%$value%'";
    ...
    public function upsert(array $data) : bool
    {
        ...
        $set = rtrim($set, ', ');
        $this->query = "INSERT INTO $this->table ($columns) VALUES ($values) ON CONFLICT DO UPDATE SET $set";
        return Application::$app->db->exec($this->query);
    }
